...
def _resolve_mode_dir(root: Path, scenario_id: str, route_id: int, mode: str) -> Optional[Path]:
    sc_dir = root / "scenario_runs" / scenario_id
    mode_l = str(mode or "").lower()
    candidate_bases = [
        sc_dir / f"matrix_out_{mode_l}" / "runs",
        sc_dir / "matrix_out" / "runs",
    ]
    ...
    with path.open("r") as f:
        for line in f:
            ...
            if isinstance(j, dict):
                j["_raw_bytes"] = len(line.encode("utf-8", errors="ignore"))
            yield j
...
def main() -> None:
    ...
    drone_middleware_rows: List[dict] = []
    ...
    for root in run_dirs:
        ...
        for r in res_rows:
            ...
            for j in _iter_jsonl(evt_file):
                ...
                if et.startswith("f2.drone_context.") or et in {
                    "f2.downstream_apply_guard.drone_skip",
                    "f2.strict_b1_floor.apply_drone_guard",
                }:
                    request_id = str(j.get("request_id", j.get("req_id", "")) or "")
                    provider_id = str(j.get("provider_id", "") or "")
                    requester_tls = str(j.get("requester_tls", j.get("tls_id", "")) or "")
                    decision_source = str(j.get("decision_source", "") or "")
                    selected_action = str(j.get("selected_action", "") or "")
                    target_edges = j.get("target_edges", j.get("lookahead_edges", []))
                    if isinstance(target_edges, list):
                        target_edges_s = ";".join(str(x) for x in target_edges)
                    else:
                        target_edges_s = str(target_edges or "")
                    raw_bytes = _to_float(j.get("_raw_bytes"), default=None)
                    payload_size = _to_float(j.get("payload_size_bytes"), default=None)
                    metrics = j.get("metrics", {})
                    if isinstance(metrics, dict) and payload_size is None:
                        payload_size = _to_float(metrics.get("payload_size_bytes"), default=None)
                    ...
                    base_drone_row = {
                        ...
                        "ev_id": ev_id,
                        "requester_tls": requester_tls,
                        "tls_id": tls_id or requester_tls,
                        "provider_id": provider_id,
                        "target_edges": target_edges_s,
                        "sim_time": sim_time,
                        "decision_source": decision_source,
                        "selected_action": selected_action,
                        "blocked": _to_int(j.get("blocked"), default=None),
                        "reason": str(j.get("reason", "") or ""),
                        "worst_edge": str(j.get("worst_edge", "") or ""),
                        "confidence": _to_float(j.get("confidence"), default=None),
                        "decision_deadline_sec": _to_float(j.get("decision_deadline_sec"), default=None),
                    }

                    def _append_drone_metric(metric_name: str, metric_value: Optional[float], unit: str) -> None:
                        if metric_value is None:
                            return
                        drone_middleware_rows.append(
                            {
                                **base_drone_row,
                    ...
                    _append_drone_metric("message_count", 1.0, "count")
                    _append_drone_metric("payload_size_bytes", payload_size if payload_size is not None else raw_bytes, "bytes")
                    _append_drone_metric(
                        "drone_task_acceptance_latency_ms",
                        _to_float(j.get("request_latency_ms"), default=None),
                        "ms",
                    )
                    _append_drone_metric(
                        "drone_response_latency_ms",
                        _to_float(j.get("response_latency_ms"), default=None),
                        "ms",
                    )
                    _append_drone_metric(
                        "observation_freshness_age_ms",
                        _to_float(j.get("context_age_ms"), default=None),
                        "ms",
                    )
                    if et == "f2.drone_context.stale":
                        _append_drone_metric("stale_assessment_rejection", 1.0, "count")

                if et == "discovery.query.response":
                    discovery_latency = _to_float(j.get("latency_ms"), default=None)
                    if discovery_latency is None:
                        discovery_latency = _to_float(j.get("response_latency_ms"), default=None)
                    if discovery_latency is None:
                        discovery_latency = _to_float(j.get("duration_ms"), default=None)
                    if discovery_latency is not None:
                        drone_middleware_rows.append(
                            {
                                "dataset": dataset,
                                "scenario_id": scenario_id,
                                ...
                                "target_edges": "",
                                "sim_time": sim_time,
                                "decision_source": "discovery_service",
                                "selected_action": "capability_discovery",
                                "blocked": None,
                                "reason": str(j.get("reason", "") or ""),
                                "worst_edge": "",
                                "confidence": None,
                                "decision_deadline_sec": None,
                                "metric_name": "capability_discovery_latency_ms",
                                "metric_value": float(discovery_latency),
                                "unit": "ms",
                            }
                        )
        ...
        # Parse FNM sidecar logs (scenario-level). Current matrix wrappers isolate
        # sidecars by mode using fnm_sidecars_b0/b1/f2; older runs used fnm_sidecars.
        for sc_dir in sorted((root / "scenario_runs").glob("*")):
            ...
            fnm_dirs = [p for p in sorted(sc_dir.glob("fnm_sidecars*")) if p.is_dir()]
            for fnm_dir in fnm_dirs:
                mode_guess = ""
                parts = fnm_dir.name.split("_")
                if parts and parts[-1].upper() in {"B0", "B1", "F2", "F2P", "F2D", "F2PD", "F1", "F3"}:
                    mode_guess = parts[-1].upper()
    ...
    route_fnm: Dict[Tuple[str, str, int, str], List[float]] = defaultdict(list)
    ...
    for r in fnm_lat_rows:
        ...
        if evt and evt not in fnm_events_for_latency:
            continue
        route_fnm[(ds, sc, rt, md)].append(d_ms)
    ...
    _write_csv(
        out_dir / "drone_middleware_metrics.csv",
        drone_middleware_rows,
        [
            "dataset",
            "scenario_id",
            "route_id",
