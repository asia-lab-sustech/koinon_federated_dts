...
def _script_fingerprint(path: Path) -> Dict[str, object]:
    out: Dict[str, object] = {
        "path": str(path),
...
def _parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--mqtt-host", default="", help="optional override for FNM internal/federation broker host")
    ap.add_argument("--mqtt-port", type=int, default=0, help="optional override for FNM internal/federation broker port")
    ...
    ap.add_argument("--generator-f2d-contextual-subscriptions-enable", action="store_true", default=False)
    ap.add_argument("--generator-f2d-contextual-back-nodes", type=int, default=2)
    ap.add_argument("--generator-f2d-contextual-forward-nodes", type=int, default=6)
    ap.add_argument("--generator-f2d-contextual-back-edges", type=int, default=2)
    ap.add_argument("--generator-f2d-contextual-forward-edges", type=int, default=8)
...
def _rewrite_cfg_mqtt_broker(src_cfg: Path, dst_cfg: Path, host: str, port: int) -> Path:
    """Write a runtime FNM config with internal/federation MQTT broker overrides."""
    if yaml is None:
        raise SystemExit("PyYAML is required for --mqtt-host/--mqtt-port sidecar config override")
    try:
        raw = yaml.safe_load(src_cfg.read_text(encoding="utf-8"))
    except Exception as e:
        raise SystemExit(f"Invalid FNM config YAML: {src_cfg} ({e})") from e
    if not isinstance(raw, dict):
        raise SystemExit(f"Invalid FNM config YAML structure: {src_cfg}")
    cfg = dict(raw)
    node = dict(cfg.get("node") or {})
    comm = dict(node.get("communication") or {})
    for bus_key in ("internal_event_bus", "federation_event_bus"):
        bus = dict(comm.get(bus_key) or {})
        broker = dict(bus.get("broker") or {})
        if str(host or "").strip():
            broker["host"] = str(host).strip()
        if int(port or 0) > 0:
    ...
    dst_cfg.write_text(yaml.safe_dump(cfg, sort_keys=False, allow_unicode=False), encoding="utf-8")
    return dst_cfg
...
def _wait_ev_preflight(
    *,
    ...
    t0 = float(time.time())
    # Allow disabling state-pull gating with 0 (useful when EV HTTP server
    # is expected to come up after sidecars start).
    min_state_ok = max(0, int(min_state_ok))
...
def main() -> None:
    ...
    fp = _script_fingerprint(fnm_script)
    print(
        "FNM_SCRIPT_INFO "
        f"path={fp.get('path')} exists={int(bool(fp.get('exists')))} "
        f"size={fp.get('size_bytes')} sha256_12={fp.get('sha256_12') or '-'} "
        f"has_fcm_class={int(bool(fp.get('has_fcm_class')))} "
        f"has_fcm_config_event={int(bool(fp.get('has_fcm_config_event')))} "
        f"has_fcm_query_event={int(bool(fp.get('has_fcm_query_event')))}"
    )
    ...
    if bool(args.auto_generate_intersection_configs):
        ...
        if bool(args.generator_f2d_contextual_subscriptions_enable):
            gen_cmd += [
                "--f2d-contextual-subscriptions-enable",
                "--f2d-contextual-back-nodes",
                str(int(args.generator_f2d_contextual_back_nodes)),
                "--f2d-contextual-forward-nodes",
                str(int(args.generator_f2d_contextual_forward_nodes)),
                "--f2d-contextual-back-edges",
                str(int(args.generator_f2d_contextual_back_edges)),
                "--f2d-contextual-forward-edges",
                str(int(args.generator_f2d_contextual_forward_edges)),
            ]
        if args.dry_run:
    ...
    mqtt_override_enabled = bool(str(args.mqtt_host or "").strip() or int(args.mqtt_port or 0) > 0)
    runtime_cfg_dir = log_dir / "runtime_configs"

    def _effective_cfg(path: Path) -> Path:
        if not mqtt_override_enabled:
            return path.resolve()
        return _rewrite_cfg_mqtt_broker(
            path.resolve(),
            runtime_cfg_dir / path.name,
            str(args.mqtt_host or "").strip(),
            int(args.mqtt_port or 0),
        )

    if str(args.ev_config).strip():
        ev_cfg = Path(args.ev_config).resolve()
        ev_cfg_effective = _effective_cfg(ev_cfg)
        proc_specs.append(
            {
                "name": "fnm_ev",
                "cfg": str(ev_cfg_effective),
    ...
    for spec in proc_specs:
        ...
        procs.append(p)
        print(
            f"START name={spec['name']} pid={p.pid} cfg={spec['cfg']} jsonl={spec['jsonl']} "
