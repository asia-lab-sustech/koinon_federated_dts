    ...
    for p in paths:
        for rec in _read_jsonl(p):
                    ...
                    samples.append(
                        {
                            ...
                            "key": req_id or trace_id or _corr_key(rec, ["ev_id", "tls_id", "source_service"]),
                            "latency_wall_ms": float(dms),
                            "latency_sim_ms": None,
            ...
            if et in ("ev.request.received", "ev.request.in"):
                ...
                if ev_id and tls_id:
                    k_ev = f"{ev_id}|{tls_id}"
                    if k_ev not in ev_req_to_apply_start:
                        ev_req_to_apply_start[k_ev] = (ts_wall, ts_sim, rec)
                    if k_ev not in ev_req_to_signal_start:
...
def main() -> int:
    ...
    print(json.dumps({"out_dir": out_dir, "summary": summary_path, "samples_csv": samples_csv, "metrics_csv": metrics_csv}))
    return 0
